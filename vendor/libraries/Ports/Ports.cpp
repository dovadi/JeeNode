...
// $Id: Ports.cpp 4887 2010-03-22 23:09:59Z jcw $
...
word HeadingPlug::adcValue(byte type) const {
    aux.digiWrite(1);
    adc.send();
    adc.write(0xFF);
    adc.write(0xE0 | (type << 4));
    adc.stop();
    delay(40);
    adc.send();
    adc.write(0xFD);
...
void HeadingPlug::readout(int& temp, int& pres) const {
    word D1 = adcValue(1);
    // Serial.print("D1 = ");
    // Serial.println(D1);
    word D2 = adcValue(0);
